            ...
            <nav class="flex-1 px-4 py-6 space-y-6 overflow-y-auto">
                <div>
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('admin.reports.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50/30 text-emerald-700 font-bold relative group border border-emerald-100">
                        ...
                        <form action="{{ route('admin.reports.index') }}" method="GET" class="flex flex-wrap items-center gap-2">
                            <div class="relative">
                                <input type="text" name="search" value="{{ request('search') }}" 
                                       ...
                                       class="text-sm rounded-xl border-slate-200 shadow-sm py-2 pl-9 pr-3 focus:ring-emerald-500 focus:border-emerald-500 w-48">
                            ...
                            @if(isset($categories) && $categories->count() > 0)
                                <select name="category" class="text-sm font-semibold rounded-xl border-slate-200 shadow-sm py-2 pl-3 pr-8 focus:ring-emerald-500 focus:border-emerald-500 appearance-none bg-white">
                            ...
                            <select name="sort" class="text-sm font-semibold rounded-xl border-slate-200 shadow-sm py-2 pl-3 pr-8 focus:ring-emerald-500 focus:border-emerald-500 appearance-none bg-white">
                            ...
                            @if(request()->hasAny(['search', 'status', 'category', 'sort']))
                                <a href="{{ route('admin.reports.index') }}" class="text-xs text-red-500 font-bold hover:underline px-2">
                                    Reset
                                </a>
                            @endif
                        </form>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-100">
                            <thead class="bg-slate-50/70">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Pelapor</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Detail Masalah</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Aksi & Status</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="hover:bg-emerald-50/30 transition-colors">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div class="text-sm font-bold text-slate-800">{{ $report->user->name ?? 'User' }}</div>
                                            ...
                                            <div class="text-[10px] text-slate-400 mt-2 ml-13 flex items-center gap-1">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="mb-2">
                                                <span class="inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-extrabold rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 uppercase tracking-wider">
                                                    {{ $report->category->name ?? 'Umum' }}
                                                </span>
                                            </div>
                                            <div class="text-sm font-bold text-slate-800 mt-1">{{ $report->title }}</div>
                                            <div class="text-xs text-slate-500 line-clamp-2 mt-1.5 leading-relaxed max-w-md">
                                                {{ $report->description }}
                                            </div>
                                            @if($report->photo)
                                                <div class="mt-2.5">
                                                    <a href="{{ asset('storage/' . $report->photo) }}" target="_blank" class="inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-600 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1 rounded-lg transition-colors border border-emerald-200">
                                        ...
                                        <td class="px-6 py-4">
                                            <form action="{{ route('admin.reports.updateStatus', $report->id) }}" method="POST" class="flex flex-col gap-2.5">
                                                @csrf @method('PATCH')
                                                <select name="status" class="text-xs font-bold rounded-xl border-slate-200 shadow-sm py-2.5 pl-3 pr-8 focus:ring-emerald-500 focus:border-emerald-500 appearance-none bg-white">
                                                    <option value="menunggu" {{ $report->status == 'menunggu' ? 'selected' : '' }}>🟡 Menunggu</option>
                                                    <option value="diproses" {{ $report->status == 'diproses' ? 'selected' : '' }}>🟠 Diproses</option>
                                                    <option value="selesai" {{ $report->status == 'selesai' ? 'selected' : '' }}>🟢 Selesai</option>
                                                </select>
                                                <button type="submit" class="bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-xs font-bold py-2.5 px-4 rounded-xl hover:from-emerald-700 hover:to-teal-700 transition-all shadow-md shadow-emerald-500/20 hover:scale-[1.02] flex items-center justify-center gap-1.5">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-6 py-16 text-center">
                                            ...
                                            <h4 class="text-slate-700 font-bold mb-1">Tidak Ada Laporan</h4>
                                            <p class="text-xs text-slate-400">Belum ada laporan yang masuk dari warga.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
