<div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
    ...
    <button class="p-2 text-gray-500 hover:text-gray-700 focus:outline-none transition-colors">
    ...
    <div x-show="open" 
         x-cloak
         class="absolute right-0 mt-0 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 z-[999]"
         style="display: none;">
        ...
        <div class="space-y-3 max-h-60 overflow-y-auto">
            @forelse(Auth::user()->unreadNotifications as $notification)
                <div class="p-3 bg-slate-50 rounded-xl text-xs text-slate-700">
                    <p class="font-bold">{{ $notification->data['message'] ?? 'Ada notifikasi' }}</p>
