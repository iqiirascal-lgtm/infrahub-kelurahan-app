        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-[264px] bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
                ...
                <a href="#" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-600 font-semibold hover:bg-slate-50 hover:text-emerald-600 transition-colors">
                    ...
                    <span class="ml-auto bg-slate-100 text-slate-600 py-0.5 px-2 rounded-full text-[10px] font-bold">{{ $reports->count() }}</span>
                </a>
        ...
        <div class="flex-1 flex flex-col h-full min-w-0 relative">
                ...
                <div class="flex items-center gap-3 sm:gap-5">
                    ...
                    <div x-data="{ notificationOpen: false }" class="relative">
                        ...
                        <div x-show="notificationOpen" 
                            ...
                            class="absolute right-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 z-[999]"
                            style="display: none;">
                            <h4 class="text-sm font-bold text-slate-800 mb-3 border-b pb-2">Notifikasi</h4>
                            <div class="max-h-60 overflow-y-auto">
                                <p class="text-xs text-slate-500 py-2">Belum ada notifikasi baru.</p>
                            </div>
                        </div>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pb-24 space-y-8 scroll-smooth">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:border-indigo-100 transition-colors group">
                            ...
                            <form action="{{ route('report.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div class="relative border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center bg-slate-50 hover:bg-emerald-50/50 hover:border-emerald-400 transition-colors cursor-pointer group">
                                        <svg class="w-8 h-8 text-slate-400 group-hover:text-emerald-500 transition-colors mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                    ...
                    <div class="xl:col-span-2 space-y-5">
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                            ...
                            <div class="flex gap-2 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0 hide-scrollbar">
                                <button class="px-3 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg whitespace-nowrap">Semua</button>
                                <button class="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-xs font-bold rounded-lg whitespace-nowrap">Menunggu</button>
                                <button class="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-xs font-bold rounded-lg whitespace-nowrap">Proses</button>
                                <button class="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-xs font-bold rounded-lg whitespace-nowrap">Selesai</button>
                            </div>
                        </div>
                                ...
                                <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 hover:border-emerald-300 transition-all group flex flex-col h-full">
                                    <div class="flex justify-between items-start mb-3 gap-2">
                                        <div class="flex gap-2">
                                            <span class="px-2 py-1 text-[10px] font-extrabold rounded-md bg-slate-100 text-slate-600 uppercase tracking-wider">
                                                {{ $report->category }}
                                            </span>
                                        </div>
                                    ...
                                    <div class="mt-4 flex items-center justify-between">
                                        <form action="{{ route('report.upvote', $report->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-emerald-700 hover:bg-emerald-50 px-2.5 py-1.5 rounded-lg font-bold transition-colors">
